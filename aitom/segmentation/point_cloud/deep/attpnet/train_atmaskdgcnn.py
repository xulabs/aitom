...
if opt.dataset_type == 'shapenet':
    ...
    dataset = MyMoleculeDataset(
        root_path=opt.dataset,
        npoints=opt.num_points,
        split='train')

    test_dataset = MyMoleculeDataset(
        root_path=opt.dataset,
        split='test',
        npoints=opt.num_points,
        data_augmentation=False)
...
for epoch in range(opt.nepoch):
    ...
    for i, data in enumerate(dataloader, 0):
        ...
        print('[%d: %d/%d] train loss: %f accuracy: %f' % (epoch, i, num_batch, loss.item(), correct.item() / float(tmp_bs)))
        ...
        if i % 10 == 0:
            ...
            pred_choice = pred.data.max(1)[1]
            correct = pred_choice.eq(target.data).cpu().sum()
            print('[%d: %d/%d] %s loss: %f accuracy: %f' % (epoch, i, num_batch, blue('test'), loss.item(), correct.item()/float(tmp_bs)))
    if epoch % 5 == 0:
