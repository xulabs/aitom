...
def scan(op):
    ...
    for i, (phi, theta, psi) in enumerate(op['angles']):
        ...
        print 'angle', i, 'of', len(op['angles']), '; time used', diff_time, '; time remain', remain_time                 ;           sys.stdout.flush()
...
def main():
    ...
    with open(jop['config_file']) as f:      cop = json.load(f)
    with open(jop['config_stat_out_file']) as f:      cop_out = json.load(f)

    print 'to process', len(jop['job']['angles']), 'rotation angles'        ;       sys.stdout.flush()

    sop = {'id': jop['id']}
    sop['out_dir'] = jop['out_dir']
    sop['template'] = cop_out['template_tmp']
    sop['mode'] = cop['mode']
    sop['angles'] = jop['job']['angles']
    sop['map'] = jop['job']['partition']['map_file']
