...
class PFDataset(Dataset):
    ...
    def __init__(self, csv_file, dataset_path, output_size=(240,240), transform=None):
        ...
        self.point_A_coords = self.pairs.iloc[:, 2:22].astype('float')
        self.point_B_coords = self.pairs.iloc[:, 22:].astype('float')
...
class PFPascalDataset(Dataset):
    ...
    def __init__(self, csv_file, dataset_path, output_size=(240,240), transform=None, category=None, pck_procedure='scnet'):
        ...
        self.category = self.pairs.iloc[:,2].astype('float')
    ...
    def __getitem__(self, idx):
        ...
        cur_set = int(self.category[idx])     
        sample = {'set': cur_set, 'source_image': image_A, 'target_image': image_B, 'source_im_size': im_size_A, 'target_im_size': im_size_B, 'source_points': point_A_coords, 'target_points': point_B_coords, 'L_pck': L_pck}
