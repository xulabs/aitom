...
class SpectralPooling(Layer):
    ...
    def get_config(self):
        config = {'output_size': self.output_size, 'truncation': self.truncation, 'homomorphic': self.homomorphic}
    ...
    def _dct3D(self, x):
        ...
        output = tf.transpose(dct(tf.transpose(dct(tf.transpose(dct(x_perm,2,norm='ortho'),perm=[0,1,2,4,3]),2,norm='ortho'),perm =[0,1,3,4,2]),2,norm='ortho'),perm=[0,4,3,2,1])
    ...
    def _idct3D(self, x):
        ...
        output = tf.transpose(idct(tf.transpose(idct(tf.transpose(idct(x_perm,2,norm='ortho'),perm=[0,1,4,2,3]),2,norm='ortho'),perm=[0,1,2,4,3]),2,norm='ortho'),perm=[0,2,3,4,1])
        
        return output 
 
    def _cropping3D(self, x):
        x_trunc = x[:, :self.truncation[0], :self.truncation[1], :self.truncation[2], :]
        paddings = tf.constant([[0,0], [0, self.output_size[0] - self.truncation[0]], [0, self.output_size[1] - self.truncation[1]], [0, self.output_size[2] - self.truncation[2]], [0,0]])
