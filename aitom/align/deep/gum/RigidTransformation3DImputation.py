...
class RigidTransformation3DImputation(Layer):
    ...
    def call(self, tensors, mask=None):
        ...
        if self.padding_method == "fill":
            ...
            rescale_theta = K.map_fn(self._rescale_theta, (theta, X), dtype=tf.float32)
        ...
        output = tf.cast(self._ift3d(tf.math.multiply(self._ft3d(X_t), tf.cast(M1_t, tf.complex64)) + tf.math.multiply(self._ft3d(Y), tf.cast(M2_t,tf.complex64))), tf.float32)
    ...
    def _ft3d(self, x):
        ...
        output = self._fftshift(self._fftshift(self._fftshift(tf.transpose(fft(tf.transpose(fft(tf.transpose(fft(
                                tf.cast(x_perm, tf.complex64)), perm = [0,1,2,4,3])), perm = [0,1,3,4,2])), perm = [0,4,3,2,1]), 1), 2), 3)
    ...
    def _ift3d(self, x):
        ...
        output = tf.transpose(ifft(tf.transpose(ifft(tf.transpose(ifft(self._ifftshift(self._ifftshift(self._ifftshift(
                                x_perm, 2), 3), 4)),perm = [0,1,4,2,3])), perm = [0,1,2,4,3])), perm = [0,2,3,4,1])
    ...
    def _rotation_matrix_axis(self, dim, theta):
        ...
        if dim == 0:        # x-axis
            rm = tf.stack(  [[1.0, 0.0, 0.0], [0.0, K.cos(theta), -K.sin(theta)], [0.0, K.sin(theta), K.cos(theta)]]  )
        elif dim == 1:    # y-axis
            rm = tf.stack(  [[K.cos(theta), 0.0, K.sin(theta)], [0.0, 1.0, 0.0], [-K.sin(theta), 0.0, K.cos(theta)]]  )
        elif dim == 2:        # z-axis
            rm = tf.stack(  [[K.cos(theta), -K.sin(theta), 0.0], [K.sin(theta), K.cos(theta), 0.0], [0.0, 0.0, 1.0]]  )
        else:
            raise   
    ...
    def _interpolate3d(self, imgs, x, y, z):
        ...
        base = self._repeat(tf.range(n_batch) * xlen * ylen * zlen,
                       xlen * ylen * zlen)
