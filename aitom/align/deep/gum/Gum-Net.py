...
def GUM(img_shape = [32,32,32]):
    input_shape = (img_shape[0], img_shape[1], img_shape[2], 1)        
    channel_axis = -1                                                                                                                

    shared_conv1 = Conv3D(32, (3, 3, 3), padding='valid') # feature extractors share the same weights                              
    ...
    main_input = Input(shape= input_shape, name='main_input')  # feature extraction for s_b                                                                            
    ...
    auxiliary_input = Input(shape= input_shape, name='aux_input')   # feature extraction for s_a                                                                                       
    ...
    c_ab = FeatureCorrelation()([v_a,v_b])  # correlation layer
    c_ab = FeatureL2Norm()(c_ab)
                                                                                                              
    c_ba = FeatureCorrelation()([v_b,v_a])  # correlation layer                                                                                                                  
    ...
    c = Dense(6, weights=weights)(c)  # estimated 3D rigid body transformation parameters         
    c = Activation('sigmoid')(c)                                                                                                                            

    mask_1 = Input(shape= input_shape, name='mask_1')                                                                                        
    mask_2 = Input(shape= input_shape, name='mask_2')                                                                            
                                                                                                                                                                                                        
    x, mask1, mask2 = RigidTransformation3DImputation((img_shape[0], img_shape[1], img_shape[2]))([main_input, auxiliary_input, mask_1, mask_2, c])
...
def get_transformation_output_from_model(model, x_train, y_train, mask_1, mask_2):
    ...
    get_layer_output_func = K.function([model.layers[0].input, model.layers[1].input, model.layers[-3].input, model.layers[-4].input], [model.layers[-2].output])
...
if __name__ == '__main__':
    ...
    if initial_model:
        ...
        model = load_model('gum_demo_model.h5',custom_objects={'SpectralPooling': SpectralPooling, 
                        'RigidTransformation3DImputation': RigidTransformation3DImputation, 'FeatureCorrelation': FeatureCorrelation, 
                        'FeatureL2Norm': FeatureL2Norm, 'correlation_coefficient_loss': correlation_coefficient_loss})
    else:
        model = GUM([32,32,32])        
    ...
    ### normalize data for better model adaptation ###
    
    x_test = (x_test - np.mean(x_test))/np.std(x_test)
    y_test = (y_test - np.mean(y_test))/np.std(y_test)


    ### save a copy for evaluation ###
    
    x_test_copy = x_test.copy()
    y_test_copy = y_test.copy()


    ### generate imputation masks per sample ###
    ...
    ### predict 6D transformation parameters ###
    
    transformation_output = get_transformation_output_from_model(model, x_test_copy, y_test_copy, mask_1, mask_2)
    ...
    ### finetune the model for 20 iterations ###     
    ...
    for i in range(20):
        ...
        np.random.shuffle(x_test)  # each iteration gets random pairs of subtomograms
        np.random.shuffle(y_test)              
                                                                                           
        model.fit([x_test,y_test,mask_1,mask_2], y_test, epochs=1, batch_size=32)
            

    ### predict the 6D transformation parameters ###
