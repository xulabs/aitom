...
if __name__ == '__main__':
    ...
    config = {
        ...
        'save_dir':"./output/",#save result images as .jpg file. If None -> Not save
        'dataset': '',# Directory of your Dataset
        'batch_size': 1,# batchsize, default = 1
    }
    ...
    for i, batch in enumerate(tqdm(dataloader)):
        ...
        if config['save_dir'] is not None:
            for j in range(img.shape[0]):
                torchvision.utils.save_image(img[j,0,0], os.path.join(config['save_dir'], 'img', '{}_{}.jpg'.format(i, j)))
                torchvision.utils.save_image(pred[j,0,0], os.path.join(config['save_dir'], 'mask', '{}_{}.jpg'.format(i, j)))
    if config['logdir'] is not None:
        writer.close()