...
def preprocess(d, num_of_class):
    ...
    percent_of_training = 0.8 # percentage of training data
    percent_of_labels = 0.5 # percentage of labeled data in training set
    percent_of_validation = 0.1 # percentage of validation data in training set
    ...
    for key in classes:
        ...
        for img in img_list:
            ...
            mask = np.zeros((num_of_class)) # used to control the number of the labeled samples. 
            if (1.0 * nums / len(d[key]) <= percent_of_labels * percent_of_training):
            ...
            elif ( (1.0 * nums / len(d[key])) <= percent_of_training * (1 - percent_of_validation) and (1.0 * nums / len(d[key])) > percent_of_labels * percent_of_training):
                ...
                masks.append(False)
                masks_auto.append(True)
            elif ( (1.0 * nums / len(d[key])) > percent_of_training * (1 - percent_of_validation) and (1.0 * nums / len(d[key])) <= percent_of_training):
                # validation set
                label[classes[key]] = 1
    ...
    masks = np.array(masks, dtype = np.bool) # the masks for training and validation. 
...
def run_auto_classifier(d, option, out_dir):
    ...
    x_train, x_test, data_validation = all_data # training, testing and validation data
    labels, test_labels, labels_validation = all_labels # training, testing and validation labels
    masks, masks_auto = all_masks # masks
    ...
    if option == 'train':
        ...
        adam = Adam(lr=0.0003, beta_1=0.9, decay = 0.001/500)        # choose a proper lr to control convergance speed, and val_loss
        masks_auto_training = masks_auto
        losses = {'sequential_1':"mean_squared_error", 'dense_4': "categorical_crossentropy"} # sequential_1: autoencoder output. dense_4: classifier output
        lossWeights = {'sequential_1':1.0, 'dense_4':1.0}
        model.compile(optimizer=adam, loss=losses, loss_weights=lossWeights, metrics={'sequential_1':"mean_squared_error",'dense_4':"accuracy"})
        ...
        checkpoint = ModelCheckpoint(model_autoclassifier_checkpoint_file, monitor='val_dense_4_acc', verbose=1, save_best_only=True, mode='auto')
        model.fit(x_train, [x_train, labels], epochs=1, batch_size=16, shuffle=True, sample_weight ={'dense_4': masks, 'sequential_1':masks_auto_training},\
           validation_data = (data_validation,[data_validation, labels_validation] ) ,callbacks=[checkpoint, earlyStopping])
    else:
        model = auto_classifier_model(img_shape=x_train[0].shape, num_of_class=num_of_class)
        model.load_weights(model_autoclassifier_checkpoint_file)
