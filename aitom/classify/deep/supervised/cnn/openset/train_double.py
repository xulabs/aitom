...
def main(args):
    ...
    with tf.Graph().as_default():
        ...
        with tf.device("/cpu:0"):
            ...
            softmax_next_element[0].set_shape((single_batch_size, args.image_height,args.image_width,args.image_width))
            softmax_next_element[1].set_shape(single_batch_size)
            batch_image_split = tf.split(softmax_next_element[0],1)
            batch_label_split = tf.split(softmax_next_element[1],1)
    ...
    while batch_number < args.epoch_size:
        ...
        if args.loss_type == 'lmccl':
            ...
            print('Epoch: [%d][%d/%d]\tTime %.3f\tTotal center Loss %2.3f\tTotal lmcl Loss %2.3f\tReg Loss %2.3f\tTotal softmax Loss %2.3f,\
             lr %2.5f' %
                  (epoch, batch_number+1, args.epoch_size, duration, total_err_center, total_loss_lmcl, reg_err,total_loss_softmax ,lr))
        else:
