...
def main_train(args):
    ...
    train_set = utils.dataset_from_list(args.train_data_dir,args.train_list_dir) # class objects in a list
    ...
    def _sample_people_softmax(x):  # loading the images in batches. 
        ...
        return (np.expand_dims(np.array(sample_images, dtype=np.float32),axis = 4), np.array(sample_labels,dtype=np.int32))
    ...
    with tf.Graph().as_default():
        ...
        with tf.device("/cpu:0"):
            ...
            softmax_next_element[0].set_shape((single_batch_size, args.image_height,args.image_width,args.image_width, 1))
        ...
        with slim.arg_scope([slim.model_variable, slim.variable], device="/cpu:0"):
            with tf.variable_scope(tf.get_variable_scope()) as var_scope:
                ...
                if args.loss_type == 'softmax':
                    cross_entropy_mean = utils.softmax_loss(prelogits,batch_label_split, len(train_set),1.0,reuse)
            ...
            while epoch < args.max_nrof_epochs:
                ...
                if args.loss_type == 'lmccl' or args.loss_type == 'center':
                    train_contain_center(args, sess, epoch, 
                         ...
                         losses, train_op, summary_op, summary_writer, '', centers_update_op)
                else:
...
def train_contain_center(args, sess, epoch, 
          learning_rate_placeholder, phase_train_placeholder, global_step, 
    ...
    while batch_number < args.epoch_size:
        ...
        if args.loss_type == 'lmccl':
            total_err_center, total_err_lmcl, reg_err, total_err_softmax, _, step, _ = sess.run([loss['total_loss_center'],\
             loss['total_loss_lmcl'], \
                loss['total_reg'],loss['total_loss_softmax'],\
                train_op,global_step, centers_update_op ], feed_dict=feed_dict)
            duration = time.time() - start_time
            print('Epoch: [%d][%d/%d]\tTime %.3f\tTotal center Loss %2.3f\tTotal lmcl Loss %2.3f\tReg Loss %2.3f\tTotal softmax Loss %2.3f,\
             lr %2.5f' %
                  (epoch, batch_number+1, args.epoch_size, duration, total_err_center, total_err_lmcl, reg_err,total_err_softmax ,lr))
        else:
            ...
            duration = time.time() - start_time
            print('Epoch: [%d][%d/%d]\tTime %.3f\tTotal center Loss %2.3f\tReg Loss %2.3f, lr %2.5f' %
...
def train(args, sess, epoch, 
    ...
    while batch_number < args.epoch_size:
        ...
        if arg.loss_type == 'lmcl':
            ...
            print('Epoch: [%d][%d/%d]\tTime %.3f\tTotal softmax Loss %2.3f\tReg Loss %2.3f, lr %2.5f' %
                      (epoch, batch_number+1, args.epoch_size, duration, total_err, reg_err, lr))
        batch_number += 1
    return step
