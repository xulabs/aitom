...
def gene_train_and_test(args):
    ...
    address =  args.raw_data#'/scratch/shared_data/xiangruz/classification/domain_adaptation_simulated_/snr-00_5.pickle'

    temp = np.load(address)
    datai = temp['data'].reshape(args.num_class*args.num_sample_per_class,args.image_size,args.image_size,args.image_size)
    labelsi = temp['label'].reshape(args.num_class*args.num_sample_per_class)
    #data = data - np.mean(data, axis = 0)
    #data = data / np.std(data, axis = 0) 
    #generating the training samples. 
    for i in range(args.num_class):
    	data_class = datai[args.num_sample_per_class*i:args.num_sample_per_class*i+args.num_sample_per_class, :, :, :]
