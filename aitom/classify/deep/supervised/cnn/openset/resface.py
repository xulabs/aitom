...
def resface_block(lower_input,output_channels,scope=None):
    with tf.variable_scope(scope):
        net = slim.conv3d(lower_input, output_channels,weights_initializer=tf.truncated_normal_initializer(stddev=0.01))
        net = slim.conv3d(net, output_channels,weights_initializer=tf.truncated_normal_initializer(stddev=0.01))
        return lower_input + net

def resface_pre(lower_input,output_channels,scope=None):
    net = slim.conv3d(lower_input, output_channels, stride=2, scope=scope)
    return net
...
def inference(image_batch, keep_probability, 
    ...
    with tf.variable_scope('Resface',reuse=reuse):
        with slim.arg_scope([slim.conv3d, slim.fully_connected], 
            ...
            with slim.arg_scope([slim.conv3d], kernel_size=3):
