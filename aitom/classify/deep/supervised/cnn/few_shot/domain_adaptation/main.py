...
optimizer=torch.optim.Adam(list(encoder2.parameters())+list(encoder1_S.parameters())+list(classifier.parameters()),lr=0.001)
...
if stage<1:
    ...
    optimizer=torch.optim.Adam(list(encoder2.parameters())+list(encoder1_S.parameters())+list(encoder1_T.parameters())+list(classifier.parameters()),lr=0.0002)
...
import os
result_log = opt['src_data'].split('/')[-1] + ' ' + opt['tar_data'].split('/')[-1] + ' sample=%d :%f'%(opt['n_target_samples'],max_acc)
