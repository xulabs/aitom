...
class CECT_dataset(Dataset):#需要继承data.Dataset
    ...
    def __getitem__(self, index):
        ...
        return torch.from_numpy(np.load(path)).unsqueeze(0).type(torch.FloatTensor),torch.from_numpy(np.array(self.class2id[CECT_class])).long()
...
def create_groups(X_s,Y_s,X_t,Y_t,seed=1):
    ...
    n=X_t.shape[0] #classes_num*shot
