...
class Upsample(nn.Module):
    """
    An up-sampling layer with an optional convolution.

    :param channels: channels in the inputs and outputs.
    :param use_conv: a bool determining if a convolution is applied.
    :param dims: determines if the signal is 1D, 2D, or 3D. If 3D, then
                 up-sampling occurs in the inner-two dimensions.
    """
...
class Downsample(nn.Module):
    ...
    :param use_conv: a bool determining if a convolution is applied.
    :param dims: determines if the signal is 1D, 2D, or 3D. If 3D, then
                 down-sampling occurs in the inner-two dimensions.
    """
...
def count_flops_attn(model, _x, y):
    ...
    # We perform two mat multiplications with the same number of ops.
