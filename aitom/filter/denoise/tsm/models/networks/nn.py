...
def normalization(num_channels: int) -> nn.Module:
    """
    Make a standard normalization layer with default 32 groups.

    :param num_channels: int, number of input channels.
    :return: a nn.Module for normalization.
    """
    return GroupNorm32(32, num_channels)
...
def conv_nd(dims: int, *args, **kwargs):
    ...
    :param dims: int, number of dimensions.
    """
    if dims == 1:
...
def update_params_ema(target_params, source_params, rate: float = 0.99) -> None:
    """
