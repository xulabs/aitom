# calculate normalized cross correlation between a template (masked so that the values outside the mask are NaN), at every position of a map
...
def cor(t, v):
    ...
    s_mean = FC.convolve(t=m, v=v) / n       # masked mean of v
    ss_mean = FC.convolve(t=m, v=N.square(v)) / n    # masked mean of square of v
