...
def stop_test_stat(op, fsc_stat, pass_i, cluster_modes):
    if (op['options']['stopping_test']['criterion'] == 0):
        ...
        selected_templates_min_pass_i = N.min([_fsc_stat['pass_i'] for (_, _fsc_stat) in fsc_stat[pass_i].iteritems() if (_fsc_stat['is_specific'] is None)])
...
def pursuit(self, op, data_json):
    ...
    if ('segmentation' in op):
        ...
        if ('guided_segmentation' in op['template']):
            if ('gaussian_smooth_sigma' in op['template']['guided_segmentation']):
                segmentation_tg_op['gaussian_smooth_sigma'] = op['template']['guided_segmentation']['gaussian_smooth_sigma']
            if ('phi_propotion_cutoff' in op['template']['guided_segmentation']):
                segmentation_tg_op['phi_propotion_cutoff'] = op['template']['guided_segmentation']['phi_propotion_cutoff']
    else:
    ...
    if os.path.isfile(file_stat_file):
        ...
        file_stat['data_json_file'] = data_json_file
    ...
    for pass_i in range(op['options']['pass_num']):
        ...
        if os.path.exists(dimension_reduction_dump_file):
            ...
            with open(dimension_reduction_dump_file, 'wb') as f:
                pickle.dump({'cfp_re': cfp_re, 'data_json_pca_train': data_json_pca_train, 'data_json': data_json, }, f, protocol=(-1))
            ...
            if ('kmeans' in set(cluster_modes)):
                ...
                if (not os.path.isfile(cluster_kmeans_dump_file)):
                    ...
                    with open(cluster_kmeans_dump_file, 'wb') as f:
                        pickle.dump({'cluster_ssnr_fsc': cluster_ssnr_fsc, 'kmeans_k': kmeans_k, 'kmeans_labels': kmeans_labels, 'kmeans_clusters': kmeans_clusters, }, f, protocol=(-1))
                else:
        ...
        if True:
            ...
            for _ in cas_re['selected_templates']:
                print _, '\t',
                print cas_re['tk_info'][cas_re['selected_templates'][_]['subtomogram']]['cluster_mode'], '\t',
                print cas_re['tk_info'][cas_re['selected_templates'][_]['subtomogram']]['fsc'].sum(), '\t',
                print len(cas_re['tk_info'][cas_re['selected_templates'][_]['subtomogram']]['data_json']), cas_re['selected_templates'][_]['subtomogram'][len(common_path_prefix):]
        sys.stdout.flush()
