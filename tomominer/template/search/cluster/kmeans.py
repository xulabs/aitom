...
def process(op):
    ...
    if ('PCA' in op):
        ...
        mat_km = mat
    km = SC.KMeans(n_clusters=op['kmeans']['cluster num'], n_init=op['kmeans']['n_init'], n_jobs=(op['kmeans']['n_jobs'] if ('n_jobs' in op['kmeans']) else (-1)), verbose=op['kmeans']['verbose'])
    ...
    for l in set(lbl.tolist()):
        ...
        clus_stat.append({'cluster_label': l, 'size': len([_ for _ in lbl if (_ == l)]), 'subtomogram': avg_file_name, })
