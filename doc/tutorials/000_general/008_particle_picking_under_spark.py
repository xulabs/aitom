...
def picking(path, s1, s2, t, find_maxima=True, partition_op=None, multiprocessing_process_num=0, pick_num=None):
    ...
    peaks = peak__partition(a_vt, s1=s1, s2=s2, find_maxima=find_maxima, partition_op=partition_op,
...
if __name__ == '__main__':
    path = '/Users/cobmg/Downloads/aitom_demo_single_particle_tomogram.mrc'
    ...
    spark = SparkSession \
        .builder \
        .appName("ParticlePicking") \
        .getOrCreate()

    subtom_RDD = spark.sparkContext.parallelize(mrc_header)
    print('RDD generated')
    # print(type(subtom_RDD.take(1)),type(subtom_RDD.take(1)[0]),type(subtom_RDD.take(1)[0][0]))

    # average subtomograms in parallel
    subtom_RDD.map(particle_picking).collect()

    print('Revoke PySpark SparkSession')
    spark.stop()
